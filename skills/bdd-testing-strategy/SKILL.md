...
Telemetry event emission is a first-class behaviour requirement, specced and tested alongside functional behaviour. If the feature's telemetry spec (in PRODUCT.md) says an event must fire, there must be a test that asserts it does.

**At the acceptance layer:** assert that the key telemetry events fire during the happy path and abandonment scenarios.

```typescript
// Vitest — acceptance spec with telemetry assertion
describe("user onboarding", () => {
  it("emits onboarding.completed when all steps are finished", async () => {
    const events = captureEvents(); // spy on your event emitter / analytics client
    ...
    expect(events).toContainEqual({
      ...
      properties: expect.objectContaining({
        ...
        total_duration_seconds: expect.any(Number),
      }),
    });
  });
});
...
**At the service layer:** mock the event emitter and assert it's called with the right arguments.

```typescript
describe("OnboardingService.completeStep", () => {
  it("emits onboarding.step_completed", async () => {
    const emit = vi.fn();
    const service = new OnboardingService(repo, emit);
    await service.completeStep(user, 2);
    expect(emit).toHaveBeenCalledWith("onboarding.step_completed", {
      user_id: user.id, step_number: 2, session_id: expect.any(String),
    });
  });
});
```
...
**What to test:**
- The event fires when the specified trigger occurs
- The event carries the required properties (from the telemetry spec)
- The event does NOT fire when the trigger doesn't occur (negative case)

**What not to test:** Internal event routing, analytics SDK internals, or delivery to external services — test at the emission boundary, not the infrastructure.
